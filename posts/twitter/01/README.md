...
5. **痛点占一句，不占一段，而且放在贡献之后。** post 2 是唯一例外 —— 列三个 gap 就是它全部的职责。post 3 的痛点半句 `sandboxes are heavy` **已经不在正文里**（见优先级 5 的删除清单）——对照物的角色现在由 `in a Docker container instead of a VM` 自己承担；post 6 不写痛点，直接给交付物（`You bring the agent; CUA-Lite ships the rollout loop, the sandboxes it runs in, and the eval and training stack…`）—— 早先那版 `A new project writes none of the usual plumbing — no agent loop, no VM setup, no eval scripts` 把贡献写成了三个「没有」，读者原话：「我们这个是 contribute 了一个空气吗」。
...
**Bold** `LiteSample is CUA-Lite's data schema` · `convert a dataset once, and every agent can train on it`
