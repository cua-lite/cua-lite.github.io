#!/usr/bin/env bash
# Regenerate the subset CJK webfonts for the Chinese covers (01a/01b-cover-zh).
...
set -euo pipefail
cd "$(dirname "$0")"
SRC=build/fonts_src; OUT=build/fonts
mkdir -p "$SRC" "$OUT"
...
TXT="高效框架一个开源平台提供所需的一切任何电脑轻量可扩展的沙盒内置可验证任务统数据集前沿评测支持覆盖桌面浏览器与移动端训练生态、，。：·"
